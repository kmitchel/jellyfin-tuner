    ...
    if (format === 'hls') {
        ...
        if (!session) {
            ...
            if (engine === 'qsv') {
                ...
                ffmpegArgs.push('-vf', 'yadif=0:-1:0,format=nv12,hwupload=extra_hw_frames=64,format=qsv', '-c:v', 'h264_qsv', '-b:v', '4M');
            ...
            session = {
                ...
                timer: setInterval(() => {
                    ...
                    if (sess && (Date.now() - sess.lastActivity > 60000)) {
                        console.log(`[HLS] Session timeout for ${channelNum}`);
                        sess.ffmpeg.kill();
                        sess.zap.kill();
            ...
            console.log(`[HLS] Started session for ${channelNum}`);
        } else {
    ...
    db.get("SELECT title, file_path FROM recordings WHERE id = ?", [recId], (err, rec) => {
        ...
        if (format === 'hls') {
            ...
            if (!session) {
                ...
                if (engine === 'qsv') {
                    ffmpegArgs.unshift('-init_hw_device', 'qsv=hw', '-filter_hw_device', 'hw');
                    ffmpegArgs.push('-vf', 'yadif=0:-1:0,format=nv12,hwupload=extra_hw_frames=64,format=qsv', '-c:v', 'h264_qsv', '-b:v', '5M');
                } else if (engine === 'nvenc') {
                ...
                console.log(`[HLS File] Starting transcoding for ${sessionKey}`);
                const ffmpeg = spawn('ffmpeg', ffmpegArgs);

                ffmpeg.stderr.on('data', d => {
                    if (VERBOSE_LOGGING || d.toString().includes('Error')) console.log(`[HLS] ${d.toString()}`);
                });
                ...
                session = {
                    ...
                    timer: setInterval(() => {
                        ...
                        if (sess && (Date.now() - sess.lastActivity > 300000)) { // 5 min timeout for VOD
                            console.log(`[HLS] Session timeout for ${sessionKey}`);
                            sess.ffmpeg.kill();
                            clearInterval(sess.timer);
                            try { fs.rmSync(sess.dir, { recursive: true, force: true }); } catch (e) { }
