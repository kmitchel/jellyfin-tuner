...
const EPG = {
    ...
    async grab(scanTimeout = 15000) {
        ...
        if (freeTuners.length === 0) {
            console.log('[EPG] No tuners available, skipping scan.');
        ...
        try {
            console.log(`[EPG] Starting parallel scan on ${freeTuners.length} tuners...`);
            ...
            const runScanner = async (tuner) => {
                while (queue.length > 0) {
                    ...
                    console.log(`[EPG] Tuner ${tuner.id} -> ${freq} Hz (${channelName})`);
                    try {
                        await this.scanMux(tuner, channelName, freq, scanTimeout);
                    } catch (e) {
                        console.error(`[EPG] Error on Tuner ${tuner.id} at ${freq}:`, e);
                    }
            ...
            this.isInitialScanDone = true;
            console.log('[EPG] All parallel scans complete.');
        }
    },
