    ...
    if (type === 'dvr') {
        for (let i = 0; i < TUNERS.length; i++) {
            ...
            if (tuner.usageType === 'live' || tuner.usageType === 'epg' || tuner.epgScanning) {
                console.log(`[Tuner] DVR is preempting ${tuner.usageType || 'EPG'} on Tuner ${tuner.id}`);
        ...
        for (let i = 0; i < TUNERS.length; i++) {
            ...
            if (tuner.usageType === 'epg' || tuner.epgScanning) {
                console.log(`[Tuner] Live is preempting EPG on Tuner ${tuner.id}`);
                if (tuner.killSwitch) tuner.killSwitch();
        ...
        if (ENABLE_PREEMPTION) {
            for (let i = 0; i < TUNERS.length; i++) {
                ...
                if (tuner.usageType === 'live') {
                    console.log(`[Tuner] Preempting existing Live stream on Tuner ${tuner.id}`);
                    if (tuner.killSwitch) tuner.killSwitch();
